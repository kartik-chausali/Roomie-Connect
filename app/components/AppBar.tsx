...
export default function AppBar(){
  ...
  const[bgColor , setbgColor] = useState('tansparent');
  ...
  useEffect(()=>{
    const handleScroll = ()=>{
      ...
      if(scrollY > 50){
        setbgColor('bg-gradient-to-b from-nav-blendColor via-black to-nav-blendColor shadow-[2px_4px_16px_0px_rgba(248,248,248,0.06)_inset]')
      }else setbgColor('')
    }
...
//             <span className="text-white font-semibold font-sans text-xs sm:text-lg">Roomie<span className="text-logo-color-green">Connect</span></span>
//             </div>
//             <Navigation/>
...
return <div className={`fixed inset-x-0 top-4 z-50 mx-auto w-[95%] max-w-7xl lg:w-full flex  justify-between ${bgColor} rounded-xl px-2 py-2  `}>
          <div className="flex items-center">
           <Image src={roomate} alt="room" className="h-10 w-10 rounded-lg"/>
             <span className="text-white font-medium font-sans text-xs sm:text-sm">Roomie<span className="text-logo-color-green">Connect</span></span>
       </div>
